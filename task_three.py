...
def addNewField(customers, headers, new_field_name, fields_to_combine, operation, format=None):
...
for customer, orders in customer_orders.items():
    for i in range(len(orders)):
        for field in number_fields:
            orders[i] = orders[i][:order_headers.index(field)] + (eval(orders[i][order_headers.index(field)]),) + orders[i][order_headers.index(field)+1:]
